...
*   **Scheduling:** Administra y reordena la cola de solicitudes de E/S para optimizar el rendimiento y minimizar los tiempos de acceso al hardware.
*   **Buffering**
*   **Caching**
...
### 8.1 Scheduling de E/S (Planificación de Disco)
La planificación de E/S busca gestionar de manera eficiente la cola de solicitudes enviadas a los dispositivos de almacenamiento (especialmente discos rígidos / HDD), minimizando los tiempos mecánicos de acceso.

#### Tiempos involucrados en E/S a disco:
*   **Tiempo de búsqueda (*seek time*):** El tiempo más crítico; es el tiempo necesario para posicionar la cabeza lectora/escritora sobre el cilindro/pista correspondiente.
*   **Latencia rotacional:** El tiempo que tarda el disco en rotar hasta que el sector deseado queda bajo la cabeza lectora.
*   **Ancho de banda:** La cantidad total de bytes transferidos dividida por el tiempo total entre la primera solicitud y la última transferencia.

#### Políticas / Algoritmos de Scheduling de Disco:
*   **FCFS / FIFO (*First Come, First Served*):**
    *   **Mecánica:** Atiende las solicitudes en el orden en que llegan a la cola.
    *   **Ventaja:** Sencillo de implementar e imparcial (no produce inanición).
    *   **Desventaja:** Ineficiente. La cabeza oscila salvajemente entre pistas distantes ("como bola sin manija"), resultando en un alto *seek time*.
*   **SSTF (*Shortest Seek Time First*):**
    *   **Mecánica:** Selecciona la solicitud que requiera el menor movimiento de la cabeza desde su posición actual (algoritmo goloso).
    *   **Ventaja:** Reduce sustancialmente el tiempo de búsqueda promedio en comparación con FCFS.
    *   **Desventaja:** No es óptimo y puede provocar **inanición (*starvation*)** en solicitudes lejanas si continúan llegando peticiones cercanas.
*   **SCAN (Algoritmo del Ascensor):**
    *   **Mecánica:** La cabeza se mueve continuamente de un extremo a otro del disco, atendiendo las solicitudes que encuentra en la dirección actual. Al llegar al extremo, invierte el sentido y atiende las demás.
    *   **Ventaja:** Evita la inanición y ofrece una distribución de tiempo de espera más uniforme.
    *   **Desventaja:** Las solicitudes que ingresan justo detrás del paso de la cabeza deben esperar a que esta realice el recorrido completo de ida y vuelta.

> **Nota:** En la práctica, los sistemas operativos combinan estas políticas con esquemas de prioridades (por ejemplo, para swapping de páginas de memoria o vaciado de cachés).
...
## 11. Redundancia de Almacenamiento: RAID
**RAID** (*Redundant Array of Inexpensive Disks*) es un esquema que combina múltiples discos rígidos físicos en una única unidad lógica. Sus objetivos principales son mejorar el rendimiento (ancho de banda) y/o brindar tolerancia a fallos mediante redundancia de datos.

### 11.1 Tácticas Básicas
*   **Striping (Distribución por bandas):** Divide un archivo en bloques y los almacena alternadamente en distintos discos. Permite lecturas/escrituras en paralelo, aumentando el ancho de banda.
*   **Mirroring (Espejado):** Duplica los datos exactamente en dos o más discos. Si un disco falla, el sistema continúa funcionando de manera transparente con la copia.
*   **Paridad:** Almacena información matemática calculada (normalmente con XOR) para poder reconstruir los datos de un disco que falle, evitando tener que duplicar el 100% del espacio.

### 11.2 Niveles de RAID
*   **RAID 0 (Striping puro):**
    *   **Mecánica:** Distribuye los bloques alternadamente entre los discos.
    *   **Ventaja:** Máximo rendimiento y uso del 100% de la capacidad total.
    *   **Desventaja:** Sin redundancia. La falla de un solo disco implica la pérdida total de los datos.
*   **RAID 1 (Mirroring puro):**
    *   **Mecánica:** Espeja completamente los datos en dos o más discos.
    *   **Ventaja:** Alta tolerancia a fallos y mejor rendimiento en lecturas concurrentes.
    *   **Desventaja:** Costoso (50% de capacidad útil) y las escrituras requieren actualizar ambos discos.
*   **RAID 0+1 y RAID 10 (Combinados):**
    *   **Mecánica:** Combinan la velocidad del *striping* con la seguridad del *mirroring*.
    *   **Diferencia:** RAID 10 (*1+0*) espeja pares de discos y luego aplica *striping*, siendo mucho más seguro ante fallas múltiples que RAID 0+1.
*   **RAID 2 y RAID 3 (Paridad a nivel de bit/byte):**
    *   **Mecánica:** Utilizan códigos de error (ECC en RAID 2) o un disco dedicado de paridad a nivel de byte (RAID 3). Todos los discos participan en cada operación de E/S.
*   **RAID 4 (Paridad dedicada por bloques):**
    *   **Mecánica:** Aplica *striping* a nivel de bloque y reserva un disco exclusivo para la paridad.
    *   **Desventaja:** El disco de paridad es un cuello de botella crítico para las escrituras.
*   **RAID 5 (Paridad distribuida):**
    *   **Mecánica:** Distribuye tanto bloques de datos como la paridad entre todos los discos ($N+1$).
    *   **Ventaja:** Elimina el cuello de botella de RAID 4. Soporta la falla de 1 disco. Es la opción más utilizada en servidores.
    *   **Desventaja:** Durante la reconstrucción tras reemplazar un disco roto, el rendimiento se degrada notablemente.
*   **RAID 6 (Doble paridad distribuida):**
    *   **Mecánica:** Agrega un segundo bloque de paridad también distribuido entre los discos.
    *   **Ventaja:** Tolera la falla simultánea de hasta 2 discos rígidos.

> **Importante:** RAID brinda **tolerancia a fallos de hardware** y alta disponibilidad, pero **no reemplaza a las copias de seguridad (backups)**. Si se borra o corrompe un archivo por software, RAID replicará esa eliminación en todos los discos.
...
| **Scheduling de E/S** | Reordena la cola de solicitudes (FCFS, SSTF, SCAN) para minimizar el tiempo de búsqueda (*seek time*) en disco. |
| **API del driver** | Conjunto de funciones (init, open, close, read, write, remove) que el driver expose al kernel. |
| **RAID** | Arreglo redundante de discos que combina *striping* (rendimiento), *mirroring* y/o paridad (tolerancia a fallos). No reemplaza a los backups. |
